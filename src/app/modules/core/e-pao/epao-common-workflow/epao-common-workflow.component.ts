...
@Component({
    selector: 'app-epao-common-workflow',
    templateUrl: './epao-common-workflow.component.html',
    styleUrls: ['./epao-common-workflow.component.css']
})
export class EPAOCommonWorkflowComponent implements OnInit {
    ...
    constructor(private elem: ElementRef,
        ...
        public dialogRef: MatDialogRef<EPAOCommonWorkflowComponent>,
        @Inject(MAT_DIALOG_DATA) public data,
        private el: ElementRef,
        private commonWorkflowService: EpaoCommonWorkflowService
    ) { }
     ...
     */
    createForm() {
        // console.log(this.selectedAction.sendBackFlag)
        this.workflowForm = this.fb.group({
            
            workflowAction: ['', Validators.required],
            remarks: ['', 
            this.conditionalValidator(
                (() => this.selectedAction && this.selectedAction.sendBackFlag !== 0),
                    Validators.required
              )
            ]
        });
    ...
    formControlValueChanged() {
        const remarkControl = this.workflowForm.get('remarks');
        this.workflowForm.get('workflowAction').valueChanges.subscribe(
            (mode) => {
                console.log(mode);
                if (mode === 12) {
        ...
        try {
            if (selectedAction.locationTypeName) {
                if (selectedAction.locationTypeName === DataConstant.CUSTOM_LOCATION_TYPE) {
                    if (this.data.menuModuleName === ModuleNames.EPAO) {
...
/**
 * @description To get CO Office for Epao
 */
                    ...
                    if (res === 'yes') {
                        if (this.workflowForm.valid && !this.isSubmitted && this.apiErrorMessageList.length === 0) {
                            ...
                            if (!this.isOnlyDummyEntry) {
                                ...
                                } else if (this.isSameOffice) {   // added by shailesh
                                    if (!this.isUserVisible) {
                                        // tslint:disable-next-line:max-line-length
                                        params['wfDetails'][0]['assignedTo']['officeId'] = this.officeId;
                                        params['wfDetails'][0]['assignedTo']['wfRoleId'] = this.selectedAction.toWfRoleId;
