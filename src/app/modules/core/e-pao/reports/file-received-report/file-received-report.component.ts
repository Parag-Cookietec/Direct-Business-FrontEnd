...
import { FileRecReport } from 'src/app/models/e-pao/epaoModel';
...
@Component({
  selector: 'app-file-received-report',
  templateUrl: './file-received-report.component.html',
  styleUrls: ['./file-received-report.component.css']
})
export class FileReceivedReportComponent implements OnInit {
 ...
 @ViewChild(MatPaginator) set matPaginator(mp: MatPaginator) {
  ...
  newdisplayedColumns: string[] = [ 'fileName', 'date', 'eodClosed', 'filesCount','filesAmt','downloadedFilesCount','downloadedFilesAmt',
                                    'pendingFilesCount','pendingFilesAmt'];
  ...
  fetchAllFileReceivedEntries() {
		const passData = {
			"pageIndex": this.pageIndex,
			"pageElement": this.pageSize,
			"jsonArr": this.jsonArray
		};
		this.reportsService.getFileReceivedReport(passData).subscribe((data: any) => {
		...
		this.reportsService.getFileReceivedReport(passDataPrint).subscribe((data: any) => {
	...
	listingData(result:any[]) {
      ...
      const data = {
				fileName: 'file1',
        date: '2-Jul-2020',
        eodClosed: element.eodClosed,
        filesCount: element.filesToBeDownloadedCount,
        filesAmt: element.filesToBeDownloadedAmount,
        downloadedFilesCount:element.filesDownloadedCount,
        downloadedFilesAmt:element.filesDownloadedAmount,
        pendingFilesCount:element.pendingToBeDownloadedCount,
        pendingFilesAmt: element.pendingToBeDownloadedAmount
      }
	...
	listingDataExcel(result:any[]) {
		...
		result.forEach(element => {
			const data = {
				'File Name': 'file1',
        'Date': '2-Jul-2020',
        'Eod Closed': element.eodClosed,
        'Files To Be Downloaded Count': element.filesToBeDownloadedCount,
        'Files To Be Downloaded Amount': element.filesToBeDownloadedAmount,
        'Downloaded Files Count': element.filesDownloadedCount,
        'Downloaded Files Amount': element.filesDownloadedAmount,
        'Pending Files Count': element.pendingToBeDownloadedCount,
        'Pending Files Amount': element.pendingToBeDownloadedAmount
			}
			Element_Data.push(data);
		});
		return Element_Data;
	...
	searchDisplay() {
		...
		if(cnt>1){
			...
			this.fetchAllFileReceivedEntries();
		}
	}
	...
	downloadPDF() {
		...
		setTimeout(()=>{
			...
			html2canvas(content).then(canvas => {
				...
				doc.save('file_received_report.pdf');
				this.printing = false;
			})
		},1000)
	}
	downloadExcel(){
		...
		FileSaver.saveAs(data, 'file_received_report_exported'+ EXCEL_EXTENSION);
	}
