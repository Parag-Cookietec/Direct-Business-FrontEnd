...
import { TalukaList, AttachmentTypeList, ClaimEntryListingSendForPayment } from '../../models/doiModel';

@Component({
  selector: 'app-claim-entry-listing-send-for-payment',
  templateUrl: './claim-entry-listing-send-for-payment.component.html',
  styleUrls: ['./claim-entry-listing-send-for-payment.component.css']
})
export class ClaimEntryListingSendForPaymentComponent implements OnInit {

  // date
  todayDate = new Date();
  // form group
  claimEntryListingSendForPaymentForm: FormGroup;
  // form control
  ...
  elementData: ClaimEntryListingSendForPayment[] = [
    {
  ...
  dataSource = new MatTableDataSource<ClaimEntryListingSendForPayment>(this.elementData);
  // table end
  ...
  ngOnInit() {
    this.claimEntryListingSendForPaymentForm = this.fb.group({
      policyNo: [''],
      insuredName: [''],
      district: [''],
      taluka: [''],
  ...
  // reset form
  reset() {
    this.claimEntryListingSendForPaymentForm.reset();
  }

  // select taluka on basis of district
  selectDistrict() {
    const district = this.claimEntryListingSendForPaymentForm.value.district;
