...
@Component({
  selector: 'app-avaiation-policy',
  templateUrl: './avaiation-policy.component.html',
  styleUrls: ['./avaiation-policy.component.css']
})
export class AvaiationPolicyComponent implements OnInit {
  ...
  aircraftsDS = new MatTableDataSource<any>(this.aelementData);
  ...
  constructor(private fb: FormBuilder, private aviationService: AviationService) { }
  ...
  submit(saveType: number) {
    const dataToSubmit = {
      ...
      "tdoiDbPolAviationAircrafts": this.getAircraftsDetails(),
      ...
      "tdoiDbPolAviationRiDtls": this.getRiDetails()
    }

    this.aviationService.createData(dataToSubmit,'policy').subscribe(res=>{});
