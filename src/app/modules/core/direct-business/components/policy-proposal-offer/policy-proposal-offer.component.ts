...
@Component({
  selector: 'app-policy-proposal-offer',
  templateUrl: './policy-proposal-offer.component.html',
  styleUrls: ['./policy-proposal-offer.component.css']
})
export class PolicyProposalOfferComponent implements OnInit {

  // date
  todayDate = new Date();
  // form group
  policyProposalOfferForm: FormGroup;
  // form control
  ...
  ngOnInit() {
    this.policyProposalOfferForm = this.fb.group({
      proposerType: [''],
      proposerName: [''],
      policyType: [''],
      month: [''],
  ...
  reset() {
    this.policyProposalOfferForm.reset();
  }
  ...
  selectDistrict() {
    const district = this.policyProposalOfferForm.value.district;
  ...
  onView() {
    this.dataService.setOption('policy-proposal-offer', 'isView');
